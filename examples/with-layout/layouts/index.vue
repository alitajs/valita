  ...
  <van-config-provider class="valita-layout" :theme="layoutCfg?.theme||defaultTheme">
    ...
    v-if="tabActive"
      ...
      <van-tabbar-item
        ...
        :key="item?.pagePath"
        :icon="typeof getTabItemIcon(item)==='object'?undefined: getTabItemIcon(item)"
        :name="item?.pagePath"
        :badge="item?.badge"
        :dot="item?.dot">
        {{ item?.text }}
        <template #icon="props"  v-if="getTabItemIcon(item) && typeof getTabItemIcon(item)==='object'">
          <layoutCfg.tabBar.list[index].selectedIcon v-if="props.active  &&  layoutCfg?.tabBar?.list?.[index].selectedIcon && typeof layoutCfg?.tabBar?.list?.[index]?.selectedIcon==='object'"/>
          <layoutCfg.tabBar.list[index].icon  v-else-if="layoutCfg?.tabBar?.list?.[index]?.icon && typeof layoutCfg?.tabBar?.list?.[index]?.icon==='object'"/>
        </template>
      </van-tabbar-item>
    </van-tabbar>
  </van-config-provider>
...
export interface TabBarProps{
  ...
  list?: TabBarListItem[]
  tabBeforeChange?: (navigator: any,name: number | string) => void
...
const getTabItemIcon = (item: TabBarListItem)=>{
  const iconPath = state.curPagePath!==item.pagePath?item?.iconPath||item?.icon: item?.selectedIconPath||item?.selectedIcon||item?.iconPath||item?.icon
  return iconPath
}
